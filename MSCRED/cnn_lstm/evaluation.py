...
test_anomaly_score = np.zeros((util.train_end_id - util.train_start_id, 1))

# load the data from file
train_data_path = util.train_data_path
reconstructed_data_path = util.reconstructed_data_path
test_data_path = os.path.join(train_data_path, "train.npy")
...
#print(np.shape(valid_anomaly_score))
print("Max valid anom is %.2f" % max_valid_anom)
print("Threshold is %.2f" % threshold)

# compute the anomaly score in the test data.
for i in range(util.train_end_id - util.train_start_id):
	error = np.square(np.subtract(test_data[i, ..., 0], reconstructed_data[i, ..., 0]))
	#print(error)
	num_anom = np.shape(np.where(error > util.threhold/5))[1] #len(np.where(error > 0.0000001))
	#print(np.shape(np.where(error > 0.001)))
	test_anomaly_score[i] = num_anom

# plot anomaly score curve and identification result
#print(np.squeeze(test_anomaly_score))
np.savetxt(util.nameFile + '_result.csv', np.squeeze(test_anomaly_score), delimiter=',', fmt='%d')
