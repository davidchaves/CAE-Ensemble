...
def main():
    ...
    sess = tf.Session()
    data_input = tf.compat.v1.placeholder(tf.float32, [util.step_max, 2, 2, 3])

    # cnn encoder
    conv1_out, conv2_out, conv3_out, conv4_out = cnn_encoder(data_input)

    conv1_out = tf.reshape(conv1_out, [-1, 5, 2, 2, 32])
    conv2_out = tf.reshape(conv2_out, [-1, 5, 1, 1, 64])
    conv3_out = tf.reshape(conv3_out, [-1, 5, 1, 1, 128])
    conv4_out = tf.reshape(conv4_out, [-1, 5, 1, 1, 256])
    ...
    result_all = np.asarray(result_all).reshape((-1, 2, 2, 3))
