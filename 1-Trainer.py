...
for nameFile in onlyFiles:
    importedData = pd.read_csv(filesPath + nameFile, header=None, sep= " ") #2D
    ...
    logging.basicConfig(level=logging.DEBUG, filename="./Results/Log_"+ now +".txt", 
        ...
        for epoch in range(numberEpochs):
            ...
            if ((epoch > 3) and (abs(lossesCycle[-1]/lossesCycle[-2])> 10)):
                ...
                if ((epoch + 1) == numberEpochs):
                    ...
                    torch.save(encoderTrain.state_dict(), os.path.join("./Results/Models", encoderName))
                    torch.save(decoderTrain.state_dict(), os.path.join("./Results/Models", decoderName))
