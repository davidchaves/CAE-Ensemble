                ...
                if epoch % self.display_epoch == 0:
                    ...
                    with torch.no_grad():
                        ...
                        if config.validation == 3:
                            ...
                            insert_sql = """INSERT into validation (model_name, pid, settings, dataset, file_name, TN, FP, FN, 
            TP, precision, recall, fbeta, pr_auc, roc_auc, cks, validation_error, diversity_factor, beta_transfer) 
            VALUES('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', 
            '{}', '{}', '{}')""".format('CAE_ENSEMBLE_' + str(len(ensemble_member)), config.pid, "Epochs", config.dataset,
                                        Path(file_name).stem, cm[0][0], cm[0][1], cm[1][0],cm[1][1], precision, recall, fbeta,
                                        pr_auc, roc_auc, cks, error_by_timestamp, epoch, train_error)
    ...
    for ensemble_index, cae_output in enumerate(cae_outputs):
        ...
        try:
            ...
            if config.validation == 1:
                evaluated_parameter_1 = config.diversity_factor
                evaluated_parameter_2 = config.beta_transfer
            elif config.validation == 2:
                evaluated_parameter_1 = config.rolling_size
                evaluated_parameter_2 = 0
            else:
                evaluated_parameter_1 = 0
                evaluated_parameter_2 = 0
                
            if config.validation == 4:
                for i in [0.1,0.2,0.5,1,2,5,10]:
                    np_decision_K = np_decision
                    rangeTop = math.floor(-1 * len(score) * i / 100)
                    indices = np.argpartition(error, rangeTop)[rangeTop:]
                    np_decision_K[indices] = -1
                    
                    precision_K = precision_score(y_true=abnormal_segment, y_pred=np_decision_K, pos_label=pos_label)
                    recall_K = recall_score(y_true=abnormal_segment, y_pred=np_decision_K, pos_label=pos_label)
                    fbeta_K = fbeta_score(y_true=abnormal_segment, y_pred=np_decision_K, pos_label=pos_label, beta=0.5)
                    
                    insert_sql = """INSERT into validation (model_name, pid, settings, dataset, file_name, TN, FP, FN, 
                    TP, precision, recall, fbeta, pr_auc, roc_auc, cks, best_TN, best_FP, best_FN, best_TP, best_precision, 
                    best_recall, best_fbeta, best_pr_auc, best_roc_auc, best_cks, validation_error, diversity_factor,
                    beta_transfer) 
                    VALUES('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', 
                    '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')""".format(
                        'CAE_ENSEMBLE_' + str(ensemble_index), config.pid, settings, config.dataset, Path(file_name).stem, 
                        cm[0][0], cm[0][1], cm[1][0],
                        cm[1][1], precision_K, recall_K, fbeta_K, pr_auc, roc_auc, cks, cae_output.best_TN, cae_output.best_FP,
                        cae_output.best_FN, cae_output.best_TP, cae_output.best_precision, cae_output.best_recall,
                        cae_output.best_fbeta, cae_output.best_pr_auc, cae_output.best_roc_auc, train_error,
                        error_by_timestamp,config.rolling_size,i)
                    cursor_obj.execute(insert_sql)
                    conn.commit()
                ...
                insert_sql = """INSERT into validation (model_name, pid, settings, dataset, file_name, TN, FP, FN, 
                TP, precision, recall, fbeta, pr_auc, roc_auc, cks, best_TN, best_FP, best_FN, best_TP, best_precision, 
                best_recall, best_fbeta, best_pr_auc, best_roc_auc, best_cks, validation_error, diversity_factor, beta_transfer) 
                VALUES('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', 
                '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')""".format(
                    'CAE_ENSEMBLE_' + str(ensemble_index), config.pid, settings, config.dataset, Path(file_name).stem, 
                    cm[0][0], cm[0][1], cm[1][0],
                    ...
                    cae_output.best_FN, cae_output.best_TP, cae_output.best_precision, cae_output.best_recall,
                    cae_output.best_fbeta, cae_output.best_pr_auc, cae_output.best_roc_auc, cae_output.best_cks,
                    error_by_timestamp,evaluated_parameter_1,evaluated_parameter_2)
                cursor_obj.execute(insert_sql)
                conn.commit()
        ...
        for root, dirs, files in os.walk(path):
            if len(dirs) == 0:
                ...
                for file in files:
                    ...
                    if config.validation == 1: # Beta and lambda
                        ...
                        for lambda_validation in (1,2,4,8,16,32,64):
                            ...
                            metrics_result = RunValidation(file_name=file_name, config=config)
                    elif config.validation == 2 or config.validation == 4: # Window size, TopK
                        for windows_validation in (5,10,20,32,64,128,256):
